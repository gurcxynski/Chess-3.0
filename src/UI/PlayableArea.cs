    ...
    internal PlayableArea() : base(new Vector2(600, 600)) {
       ...
       AddChild(background);
        ...
        board.Pieces.ForEach((piece) => {
            ...
            {
                OnMouseReleased = (Entity entity) => OnMoved(entity as PieceIcon),
                OnMouseDown = (Entity entity) => OnPickup(entity as PieceIcon)
            };
            AddChild(icon); 
            icon.Update(drawWhiteDown);
        });
    }
    internal void OnMoved (PieceIcon icon) {
        ...
        if (bot is not null && WhiteTurn != drawWhiteDown) {
